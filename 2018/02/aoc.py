...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        for line in lines:
            ...
            match = list([x for x in lines if Levenshtein.distance(x, line) == 1])
            if match:
                # print(f"{line} {match}")
                matches.append(match)

        assert len(matches) == 2, "whoops"
        ...
        for line in lines:
            counts = Counter(line)
            twos = 1 if list(counts.values()).count(2) else 0
            threes = 1 if list(counts.values()).count(3) else 0
            # print(f"{line} {counts} {twos} {threes}")
            two_count += twos
            three_count += threes
