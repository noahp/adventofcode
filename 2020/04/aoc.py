...
def parse(x):
    return list(map(lambda x: " ".join(x.splitlines()), x.split("\n\n")))
...
def validate2(passport):
    fields = {
        "byr": (
            r"byr:(\d{4})\b",
            lambda x: int(x[0]) >= 1920 and int(x[0]) <= 2002,
        ),  # (Birth Year)
        "iyr": (
            r"iyr:(\d{4})\b",
            lambda x: int(x[0]) >= 2010 and int(x[0]) <= 2020,
        ),  # (Issue Year)
        "eyr": (
            r"eyr:(\d{4})\b",
            lambda x: int(x[0]) >= 2020 and int(x[0]) <= 2030,
        ),  # (Expiration Year)
        "hgt": (
            r"hgt:(\d+)((?:cm)|(?:in))\b",
            lambda x: hgt_val(x[0], x[1]),
        ),  # (Height)
        "hcl": (r"hcl:(#[0-9a-f]{6})\b", lambda x: True),  # (Hair Color)
        "ecl": (
            r"ecl:((?:amb)|(?:blu)|(?:brn)|(?:gry)|(?:grn)|(?:hzl)|(?:oth))\b",
            lambda x: True,
        ),  # (Eye Color)
        "pid": (r"pid:(\d{9})\b", lambda x: True),  # (Passport ID)
        # "cid",  # (Country ID)
    }
    for field, val in fields.items():
        ...
        m = re.search(val[0], passport)
        if not m:
            return False
        if not val[1](m.groups()):
            return False

    # print(' '.join(sorted(passport.split())))
    # print(re.sub(r"cid:.*?\b", "", ' '.join(sorted(passport.split()))))
...
iyr:2011 ecl:brn hgt:59in"""
        ...
        print(sum(map(validate, testvals)))
        print(sum(map(validate, vals)))
    else:
        # part 2
        testdata = """eyr:1972 cid:100
hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719"""
        ...
        # print(sum(map(validate2, testvals)))
        print(sum(map(validate2, vals)))
