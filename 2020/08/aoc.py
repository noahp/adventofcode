...
def run1(data):
    acc = 0
    runcount = list([0 for _ in range(len(data))])
    idx = 0
    while True:
        if runcount[idx] > 0:
            return False, acc
...
def run2(data):
    for idx in range(len(data)):
        ...
        if data2[idx][0] == "nop":
            data2[idx][0] = "jmp"
        elif data2[idx][0] == "jmp":
            data2[idx][0] = "nop"
        else:
            continue

        finished, acc = run1(data2)
        if finished:
            return acc

def parse(data):
    return list(
        map(lambda x: [x.split()[0], int(x.split()[1])], data.splitlines())
    )
