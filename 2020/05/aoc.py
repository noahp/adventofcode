...
def parse(input):
    return list([(x[:7], x[7:]) for x in input.splitlines()])
...
"""
        ...
        # for row, col in vals:
        #     print(compute(row, col))

        seatid = max(map(lambda x: x[2], [compute(row, col) for row, col in vals]))
        print(seatid)
    else:
        # part 2
        seatids = sorted(list(map(lambda x: x[2], [compute(row, col) for row, col in vals])))
        lastid = 0
        for seatid in seatids:
            if seatid - 2 == lastid:
                print("{} {}".format(lastid, seatid))
            lastid = seatid
