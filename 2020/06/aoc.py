...
def parse(data):
    return list([x.splitlines() for x in data.split("\n\n")])


def group_anyone(grouplines):
    return len(set("".join(grouplines)))


def group_everyone(grouplines):
    return len(set.intersection(*map(set, grouplines)))
...
b"""
    ...
    if len(sys.argv) == 2:
        print(sum(map(group_anyone, testvals)))
        print(sum(map(group_anyone, vals)))
    else:
        # part 2
        print(sum(map(group_everyone, testvals)))
        print(sum(map(group_everyone, vals)))
