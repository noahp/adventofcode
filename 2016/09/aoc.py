...
def len_of_block(data, progbar=None):
    length = 0
    marker = re.compile(r"(?P<front>.*?)\((?P<len>\d+)x(?P<cnt>\d+)\)(?P<back>.*)")
    while True:
        m = marker.match(data)
        if m:
            length += len(re.sub(r"\s", "", m.group("front")))
            # update data
            if len(sys.argv) > 2:
                # part 2, process each block too
                # TODO this is hella slow, doesn't work!
                length += len_of_block(m.group("back")[:int(m.group("len"))] * int(m.group("cnt")))
            else:
                # part 1, just add the length and proceed
                length += len(re.sub(r"\s", "", m.group("back")[:int(m.group("len"))] * int(m.group("cnt"))))
            if progbar:
                progbar.update(len(data)-len(m.group("back")[int(m.group("len")):]))
            data = m.group("back")[int(m.group("len")):]
        else:
            # no more markers
            length += len(re.sub(r"\s", "", data))
            break
    return length
