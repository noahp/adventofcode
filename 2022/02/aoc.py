...
def get_score(x):
    points = POINTS[x[1]] + score(x[0], x[1])
    # print(points)
    return points
...
def score2(data):
    ...
    y = data[1]
    if y == "X":
        return 0 + POINTS[you_lose[op.find(x)]]
    if y == "Y":
        return 3 + POINTS[you[op.find(x)]]
    if y == "Z":
        return 6 + POINTS[you_win[op.find(x)]]
...
def main():
    ...
    total_score = sum(map(get_score, data))

    print(f"part one: {total_score}")

    total_score = sum(map(score2, data))
