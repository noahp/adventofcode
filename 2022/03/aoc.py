...
def get_rucksacks(entry):
    return entry[: len(entry) // 2], entry[len(entry) // 2 :]


def get_priority(rucksacks):
    letter = set(rucksacks[0]).intersection(*rucksacks[1:])
    if letter:
        return letters.index(letter.pop()) + 1
    raise ValueError("no common letter")
...
def main():
    ...
    #     "CrZsJsPPZsGzwwsLwLmpwMDw",
    # ]
    def part_one_score(entry):
        return get_priority(get_rucksacks(entry))

    part_one = sum(map(part_one_score, data))

    print(f"part one: {part_one}")

    part_two = sum(map(get_priority, chunks(data, 3)))
